...
//	@Query(value="select * from (SELECT domain, count(*) as hits FROM url_bean group by domain order by hits desc) where rownum() < 4", nativeQuery=true)
	@Query(value="SELECT domain, count(*) as hits FROM url_bean group by domain having count(*) in (select * from (SELECT distinct count(*) as hits FROM url_bean group by domain) where rownum() < 4) order by hits desc", nativeQuery=true)
